...
from ..scalebar import get_scale
...
class MainW(QMainWindow):
    ...
    def make_buttons(self):
        ...
        self.Diameter.returnPressed.connect(self.update_diameter)
        self.Diameter.setFixedWidth(75)
        ...
        b0 += 1
        self.scale_num = "None/None"
        label = QLabel("scale (nm / pixel):")
        label.setFont(self.medfont)
        label.setToolTip(
            'you can manually enter the scale for your image, \nor press “calibrate” to let the model estimate it. \nThe size is represented by a disk at the bottom of the view window \n(can turn this disk off by unchecking “scale disk on”)'
        )
        self.segBoxG.addWidget(label, b0, 0, 1, 4)
        self.BarScale = QLineEdit()
        self.BarScale.setToolTip(
            'you can manually enter the scale for your image, \nor press “calibrate” to let the model estimate it. \nThe size is represented by a disk at the bottom of the view window \n(can turn this disk off by unchecking “scale disk on”)'
        )
        self.BarScale.setText(str(self.scale_num))
        self.BarScale.setFont(self.medfont)
        self.BarScale.returnPressed.connect(self.update_scale_num)
        self.BarScale.setFixedWidth(75)
        self.segBoxG.addWidget(self.BarScale, b0, 4, 1, 2)

        # compute diameter
        self.ScaleButton = QPushButton("calibrate")
        self.ScaleButton.setFont(self.medfont)
        self.ScaleButton.clicked.connect(self.calibrate_scale_num)
        self.segBoxG.addWidget(self.ScaleButton, b0, 6, 1, 3)
        self.ScaleButton.setEnabled(False)
        self.ScaleButton.setToolTip(
            'you can manually enter the approximate diameter for your cells, \nor press “calibrate” to let the cyto3 model estimate it. \nThe size is represented by a disk at the bottom of the view window \n(can turn this disk off by unchecking “scale disk on”)'
        )

        b0 += 1
        # choose channel
        self.ChannelChoose = QComboBox()
        self.ChannelChoose.addItems(["0: gray", "1: red", "2: green", "3: blue"])
        self.ChannelChoose.setFont(self.medfont)
        label = QLabel("chan to segment:")
        label.setFont(self.medfont)
        label.setToolTip(
            "this is the channel in which the cytoplasm or nuclei exist that you want to segment"
        )
        self.ChannelChoose.setToolTip(
            "this is the channel in which the cytoplasm or nuclei exist that you want to segment"
        )
        # self.segBoxG.addWidget(label, b0 + i, 0, 1, 4)
        # self.segBoxG.addWidget(self.ChannelChoose[i], b0 + i, 4, 1, 5)
        self.segBoxG.addWidget(label, b0, 0, 1, 4)
        self.segBoxG.addWidget(self.ChannelChoose, b0, 4, 1, 5)
    ...
    def calibrate_scale_num(self):
        scale, pix_len, image_vis, scalebar_bbox = get_scale(self.stack[self.currentZ].copy().astype(np.uint8))
        self.logger.info(f"estimated scale of scale-bar using {pix_len} pixel length = {scale} nm")
        self.scale_num = f"{scale}/{pix_len}"
        self.BarScale.setText(f"{scale}/{pix_len}")
        self.update_scale_num()
        self.progress.setValue(100)
    ...
    def update_roi_count(self):
        try:
            scale_num = eval(self.scale_num)
            self.roi_count.setText(f"{scale_num:.3f} nm/pix")
        except Exception as e:
            print(self.scale_num, e)
            self.roi_count.setText(f"Unknown Scale")
    ...
    def update_scale_num(self):
        self.compute_scale_num()

    def compute_scale_num(self):
        self.scale_num = self.BarScale.text()
        self.update_roi_count()
    ...
    def train_model(self, restore=None, normalize_params=None):
        ...
        if self.autorun:
            self.clear_all()
            self.ChannelChoose.setCurrentIndex(channels[0])
