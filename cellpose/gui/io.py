...
from ..models import mask2measurement, show_segmentation
import pandas as pd
...
def _load_image(parent, filename=None, load_seg=True, load_3D=False):
    ...
    try:
        ...
        parent.loaded = True
        print(f"GUI_INFO: loaded image: {image.shape}")
...
def _load_seg(parent, filename=None, image=None, image_file=None, load_3D=False):
    ...
    if "chan_choose" in dat:
        parent.ChannelChoose.setCurrentIndex(dat["chan_choose"][0])
...
def _save_visualization(parent):
    """ save flows and cellprob to tiff """
    filename = parent.filename
    base = os.path.splitext(filename)[0]
    measurements = mask2measurement(parent.cellpix[0])
    show_segmentation(cv2.imread(filename), parent.cellpix[0], base + "_visualization.png")
    pd.DataFrame(measurements).to_csv(base + "_measurements.csv", index=None)
    try:
        scale_num = eval(parent.scale_num)
    except Exception as e:
        print(e, parent.scale_num)
        return
    
    measurements["area (nm^2)"] = [scale_num * scale_num * v for v in measurements["area"]]
    measurements["perimeter (nm)"] = [scale_num * v for v in measurements["perimeter"]]
    measurements["equivalent_diameter (nm)"] = [scale_num * v for v in measurements["equivalent_diameter"]]
    measurements["max_diameter (nm)"] = [scale_num * v for v in measurements["max_diameter"]]
    measurements["min_diameter (nm)"] = [scale_num * v for v in measurements["min_diameter"]]
    pd.DataFrame(measurements).to_csv(base + "_measurements.csv", index=None)
...
def _save_sets(parent):
    ...
    if parent.NZ > 1:
        ...
        dat = {
            ...
            "chan_choose": [
                parent.ChannelChoose.currentIndex(),
            ],
            "filename":
                parent.filename,
            "flows":
